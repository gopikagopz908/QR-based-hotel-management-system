    ...
    <div className="mt-8 md:mt-20 space-y-10 md:px-4 sm:px-8 h-full relative">
      ...
      <div className="flex justify-center mb-6">
        <input
          ...
          className="border border-gray-300 rounded-full px-6 py-2 w-[80%] max-w-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-400"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        {categories.map((category) => (
          <button
            key={category.value}
            onClick={() => setActiveCategory(category.value)}
            className={`md:min-w-[100px] min-w-[100px] px-2 py-2 rounded-full font-semibold transition-all duration-300 text-sm ${
              activeCategory === category.value
                ? "bg-red-800 text-white"
                : "bg-gray-200 text-gray-700"
            }`}
